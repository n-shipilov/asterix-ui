...
export const SelectionService = () => {
  const { data, rowSelection, rowKey, selectedRowKeys, setSelectedRowKeys } =
    useTableContext();
  ...
  const isRowChecked = (row: any) =>
    selectedRowKeys.some((item) => item === row[rowKey!]);

  const handleRowSelect = (row: any) => {
    if (isRowChecked(row)) {
      const updatedRows = selectedRowKeys.filter(
        (selectedRow) => selectedRow !== row[rowKey!]
      );
  ...
  const handleAllRowSelect = () => {
    if (selectedRowKeys.length === data.length) {
      ...
      rowSelection?.onChange &&
        rowSelection.onChange(data.map((item) => item[rowKey!]));
    }
  };
